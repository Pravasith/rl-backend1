...
let categoryCount = 32, subCategoryCount = 84, productTypeCount = 873
...
let convertCategories = {
    ...
    handler: async (request, h) => {
        ...
        const theData = fs.readFile( __dirname + '/wood.json', "utf-8", 
        (err, res) => {
                    ...
                    subCategoryNames.push(
                        {
                            ...
                            subCategoryId : "CAT" + unitsPlaceConverter(categoryCount) + "-" + "SC" + unitsPlaceConverter(subCategoryCount),
                            productTypes: []
                        }
                    )
                    subCategoryCount++
            ...
            data.map((item, i) => {
                ...
                for(var key in item) {
                    if(item.hasOwnProperty(key)){
                        if(item[key] !== "" && item[key] !== subCategoryNames[count].subCategoryName){
            ...
            const giveProductTypeIds = () => {
                subCategoryNames.map((item, i) => {
                    item.productTypes.map((productType, j) => {
                        productType.productTypeId = item.subCategoryId + "-" + "PT" + unitsPlaceConverter(productTypeCount)
                        productTypeCount++
                    })
                })
            }
            ...
            fs.writeFile(__dirname + "/categoryRefined.json", JSON.stringify(subCategoryNames, null, 2),
                (e) => {
                    console.log("done")
                }
            )
...
let newSubcategory = {
    ...
    handler: async (request, h) => {
        ...
        if (adminPasscode === adminPassword) {
            const uploadNewSubCategory = async (categoryNameParam, categoryIdParam) => {
                ...
                if(categoryIdParam === CategoryCodes[categoryNameParam] ){
                    ...
                    await NewCategory[categoryNameParam].findOne({
                        subCategoryName,
                    })
                    .then( result => {
                    ...
                    if(productTypeAlreadyExists){
                        dataToSendBack = {
                            message : "Sorry, couldn't add the new sub-category because it was already added."
                        }
                    }
            
                    else{
                        await NewCategory[categoryNameParam].create(
                            {
                                adminId,
                                subCategoryName,
                                subCategoryId : CategoryCodes[categoryNameParam] + "-" + "SC" + unitsPlaceConverter(subCategoryCount),
                                productTypes : [],
                            }
                        )
                        .then((res) => {
                        ...
                        .then( result => {
                            ...
                            dataToSendBack = {
                                ...dataToSendBack,
                                extraMessage : "Incremented subCategory count to " + result.subCategoryCount
                            }
                    ...
                    dataToSendBack = { message: "The category name and category ID do not match" }
                }
            }
                ...
                if(categoryIdParam === CategoryCodes[categoryNameParam] ){
                    ...
                    .then( result => {
                        ...
                        result.productTypes.map((item, i) => {
                            if(item.productType.toLowerCase() === productType.toLowerCase()){
                                productTypeAlreadyExists = true
                            }
                        })
                    })
                    ...
                    if(productTypeAlreadyExists){
                        dataToSendBack = {
                            message : "Sorry, couldn't add the new productType because it was already added."
                        }
                    }
            
                    else{
                         await NewCategory[categoryNameParam].findOneAndUpdate(
                            {
                                subCategoryId,
                            },
                            {
                                $push : {
                                    productTypes : {
                                        ...
                                        "productTypeId": subCategoryId + "-" + "PT" + unitsPlaceConverter(productTypeCount),
                                        "products": []
                                    }
                                }
                            },
                        ...
                        .then((res) => {
                            dataToSendBack = {
                                ...
                                message : "New productType created successfully"
                            }
                        })
                        .catch(e => h.response(e))
                        ...
                        .then( result => {
                            ...
                            dataToSendBack = {
                                ...dataToSendBack,
                                extraMessage : "Incremented product type count to " + result.productTypeCount
                            }
                        ...
                        .then( result => {
                            ...
                            const addSubcategoryData = () => {
                                return allCategories.map(cat => {
                                    cat.subCategories.map(sCat => {
                                        if(sCat.subCategoryId === subCategoryId){
