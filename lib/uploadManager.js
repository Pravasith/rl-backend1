...
let uploadImageToS3Bucket = function (theFile, fileStorageDestination) {
    ...
    const secretAccessKeyId =
        CONFIG.awsS3Config.s3BucketCredentials.secretAccessKey
    ...
    const getImageType = () => {
        ...
        if (tempArr[0] === "regularImage") return "regularImage"
        else if (tempArr[0] === "profileImage") return "profileImage"
    }

    const getUserType = () => {
        const tempArr = imageName.split("-")

        if (tempArr[1] === "STU") return "student"
        else if (tempArr[1] === "ARC") return "architect"
        else if (tempArr[1] === "VEN") return "vendor"
        else if (tempArr[1] === "CLI") return "client"
    }
        ...
        fs.readFile(fileStorageDestination, function (error, bufferData) {
            if (error) {
                ...
                let errResp = {
                    response: {
                        message:
                            "Your file was uploaded, but our server couldn't read it :(",
                        data: {},
                    },
                    statusCode: 500,
                }
            ...
            const addBucketToParams = () => {
                if (imageType === "profileImage") {
                    params = {
                        ...params,
                        Bucket: s3BucketCredentials.bucket.images[userType]
                            .profilePictures,
                    }
                ...
                .putObject(params, function (err, data) {
                    if (err) {
                        ...
                        let error = {
                            response: {
                                message:
                                    "Error in uploading to our main server",
                                data: {},
                            },
                            statusCode: 500,
                        }
                        ...
                        if (imageType === "profileImage")
                            resolve({
                                ...
                                imageURL:
                                    CONFIG.awsS3Config.s3BucketCredentials
                                        .s3URL +
                                    s3BucketCredentials.bucket.images[userType]
                                        .profilePictures +
                                    "/" +
                                    imageName,
                            })
        ...
        Cloudinary.v2.uploader.upload(
            ...
            (error, result) => {
                if (error)
                    reject({
                        message:
                            "There has been an error in uploading to Cloudinary",
                    })
                else {
                    resolve({
                        successfullyUploaded: true,
